...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		scoreText = new FlxText(FlxG.width * 0.625, 5, 0, "", 32);
		// scoreText.autoSize = false;
		scoreText.setFormat(Paths.font("fot-rowdystd-eb.otf"), 32, FlxColor.WHITE, RIGHT);
		// scoreText.alignment = RIGHT;

		var scoreBG:FlxSprite = new FlxSprite(scoreText.x - 3, 0).makeGraphic(Std.int(FlxG.width * 0.625), 135, 0xFF000000);
		scoreBG.alpha = 0.6;
		add(scoreBG);
		...
		comboText = new FlxText(diffText.x + 178, diffText.y, 0, "", 24);
		comboText.font = diffText.font;
		add(comboText);

		showRating = new RatingDisplay(diffText.x + 223, diffText.y - 10);
		add(showRating);
	...
	override function update(elapsed:Float)
	{
		...
		if (rating != "")
			showRating.displayRating(rating);
		else
			showRating.displayRating("none");
	...
	function changeDiff(change:Int = 0)
	{
		...
		rating = Highscore.getRating(songHighscore, curDifficulty);
		#end
		diffCalcText.text = 'RATING: ${DiffCalc.CalculateDiff(songData.get(songs[curSelected].songName)[curDifficulty])}';
	...
	function changeSelection(change:Int = 0)
	{
		...
		else if (songs[curSelected].songCharacter == "gf-ratgod")
			FlxG.sound.playMusic(Paths.preview(songs[curSelected].songName), 0);
		else
