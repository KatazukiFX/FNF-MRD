...
class LoadReplayState extends MusicBeatState
{
	...
	{
		var menuBG:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menuOG'));
		...
		addWeek(["RAT's Dream", "HEART's Beat", 'One day at Lab'], 1, ['doctor']);
		...
		versionShit.setFormat(Paths.font("fot-rowdystd-eb.otf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(versionShit);

		poggerDetails = new FlxText(5, 34, 0, "Replay Details - \nnone", 12);
		poggerDetails.scrollFactor.set();
		poggerDetails.setFormat(Paths.font("fot-rowdystd-eb.otf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(poggerDetails);
		...
		if (controls.ACCEPT && grpControls.members[curSelected].text != "No Replays...")
		{
			...
			if (PlayState.rep.replay.replayGameVer == Replay.version)
			{
				...
				try
				{
					...
					{
						var diff:String = ["", "-hard"][PlayState.rep.replay.songDiff];
