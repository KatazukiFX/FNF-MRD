...
class Song
{
	...
	{
		var songLowercase = StringTools.replace(songId, " ", "-").toLowerCase();

		var songFile = '$songLowercase/$songLowercase$difficulty';

		Debug.logInfo('Loading song JSON: $songFile');

		var rawJson = Paths.loadJSON('songs/$songFile');
		var rawMetaJson = Paths.loadJSON('songs/$songLowercase/_meta');

		return parseJSONshit(songLowercase, rawJson, rawMetaJson);
	}
