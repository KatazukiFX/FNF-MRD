...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		PlayStateChangeables.confetti = FlxG.save.data.confetti;
		PlayStateChangeables.confettiOn = FlxG.save.data.confettiOn;
		...
		if (SONG == null)
			SONG = Song.loadFromJson('mad-rat-alive', '');
		...
		if (!stageTesting)
		{
			...
			if (gf.frames == null)
			{
				...
				#end
				gf = new Character(400, 130, 'gf-heart');
			}

			boyfriend = new Boyfriend(920, 450, SONG.player1);
			...
			if (boyfriend.frames == null)
			{
				...
				#end
				boyfriend = new Boyfriend(770, 450, 'bf-madrat');
			}
			...
			if (dad.frames == null)
			{
				...
				#end
				dad = new Character(100, 100, 'doctor');
			}
		}
		...
		switch (dad.curCharacter)
		{
			case 'gf-heart':
				if (!stageTesting)
					dad.setPosition(gf.x + 70, gf.y + 140);
				camPos.x += 218;
				camPos.y += 158;
				gf.visible = false;
			case 'gf-ratgod':
				if (!stageTesting)
					dad.setPosition(gf.x, gf.y + 50);
				camPos.x += 450;
				camPos.y += 10;
				gf.visible = false;
			case "heart":
		...
		kadeEngineWatermark = new FlxText(4, healthBarBG.y
			...
			16);
		kadeEngineWatermark.setFormat(Paths.font("fot-rowdystd-eb.otf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("fot-rowdystd-eb.otf"), 16, FlxColor.WHITE, FlxTextAlign.CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		judgementCounter.setFormat(Paths.font("fot-rowdystd-eb.otf"), 20, FlxColor.WHITE, FlxTextAlign.LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		judgementCounter.borderSize = 2;
		judgementCounter.borderQuality = 2;
		judgementCounter.scrollFactor.set();
		judgementCounter.cameras = [camHUD];
		judgementCounter.screenCenter(Y);
		judgementCounter.text = 'Greats: ${sicks}\nGoods: ${goods}\nEarly/Late: ${bads}\nMisses: ${misses}';
		...
		replayTxt = new FlxText(healthBarBG.x + healthBarBG.width / 2 - 75, healthBarBG.y + (PlayStateChangeables.useDownscroll ? 100 : -100), 0, "REPLAY",
			20);
		replayTxt.setFormat(Paths.font("fot-rowdystd-eb.otf"), 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botPlayState = new FlxText(healthBarBG.x + healthBarBG.width / 2 - 75, healthBarBG.y + (PlayStateChangeables.useDownscroll ? 100 : -100), 0,
			"BOTPLAY", 20);
		botPlayState.setFormat(Paths.font("fot-rowdystd-eb.otf"), 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			else if (dad.curCharacter == 'gf-heart' || dad.curCharacter == 'gf-ratgod')
				dad.dance();
...

			{
				case 0:
					...
					// screenCenter = 365
					FlxTween.linearMotion(three, 665, 185, 65, 185, Conductor.crochet / 1000, true, {
				...
				case 1:
					...
					FlxTween.linearMotion(two, 665, 185, 65, 185, Conductor.crochet / 1000, true, {
						onComplete: function(twn:FlxTween)
						{
							two.destroy();
						}
				...
				case 2:
					...
					FlxTween.linearMotion(one, 665, 185, 65, 185, Conductor.crochet / 1000, true, {
						onComplete: function(twn:FlxTween)
						{
							one.destroy();
						}
	...
	public function generateSong(dataPath:String):Void
	{
		...
		if (!paused)
		{
			...
			else if (!isStoryMode && rand < 0.01 && StringTools.replace(curSong, " ", "-").toLowerCase() == "mad-rat-alive")
				FlxG.sound.playMusic(Paths.inst2(PlayState.SONG.songId), 1, false);
			else
		...
		if (FlxG.save.data.songPosition)
		{
			...
			songName.setFormat(Paths.font("fot-rowdystd-eb.otf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic)
		{
			if (songStarted && !endingSong)
			{
				...
				if (unspawnNotes.length == 0 && notes.length == 0 && FlxG.sound.music.time / songMultiplier > (songLength - 100))
				{
					...
					else if (SONG.songId == 'one-day-at-lab')
					{
		...
		if (generatedMusic && currentSection != null)
		{
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !currentSection.mustHitSection)
			{
				...
				#end
				camFollow.setPosition(dad.getMidpoint().x + 150 + offsetX, dad.getMidpoint().y - 50 + offsetY);
				#if FEATURE_LUAMODCHART
				if (luaModchart != null)
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				{
					if (SONG.song != 'mad-rat-alive' || SONG.song != 'fermented-dream')
						camZooming = true;
	...
	function endSong():Void
	{
		...
		if (SONG.validScore)
		{
			...
			Highscore.saveRating(PlayState.SONG.songId, Ratings.GenerateLetterRank(accuracy), storyDifficulty);
			#end
		}
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					{
						GameplayCustomizeState.freeplayBf = 'bf-madrat';
						GameplayCustomizeState.freeplayDad = 'doctor';
						GameplayCustomizeState.freeplayGf = 'gf-heart';
						GameplayCustomizeState.freeplayNoteStyle = 'normal';
						GameplayCustomizeState.freeplayStage = 'cage';
						GameplayCustomizeState.freeplaySong = 'rats-dream';
				...
				{
					var diff:String = ["", "-hard"][storyDifficulty];
	...
	private function popUpScore(daNote:Note):Void
	{
		...
		var confetti1:FlxSprite = new FlxSprite();
		var confetti2:FlxSprite = new FlxSprite();
		var score:Float = 350;
		...
		if (daRating != 'shit' || daRating != 'early' || daRating != 'late')
		{
			...
			confetti1.frames = Paths.getSparrowAtlas(pixelShitPart1 + 'confettiL' + pixelShitPart2);
			confetti1.animation.addByPrefix('toss1', 'confettiL', 24, false);
			confetti1.screenCenter();
			confetti1.x = rating.x;
			confetti1.y = rating.y - 140;
			if (FlxG.save.data.distractions)
			{
				add(confetti1);
			}

			confetti2.frames = Paths.getSparrowAtlas(pixelShitPart1 + 'confettiS' + pixelShitPart2);
			confetti2.animation.addByPrefix('toss2', 'confettiS', 24, false);
			confetti2.screenCenter();
			confetti2.x = rating.x;
			confetti2.y = rating.y - 140;
			if (FlxG.save.data.distractions)
			{
				add(confetti2);
			}
			...
			{
				add(rating);
				if (PlayStateChangeables.confettiOn && daRating == 'sick')
				{
					if (!PlayStateChangeables.confetti)
						confetti1.animation.play('toss1', true);
					else
						confetti2.animation.play('toss2', true);
				}
			}
			...
			if (SONG.noteStyle != 'pixel')
			{
				...
				confetti1.setGraphicSize(Std.int(confetti1.width * 1.5));
				confetti1.antialiasing = FlxG.save.data.antialiasing;
				confetti2.setGraphicSize(Std.int(confetti2.width * 1.5));
				confetti2.antialiasing = FlxG.save.data.antialiasing;
			...
			rating.updateHitbox();
			confetti1.updateHitbox();
			confetti2.updateHitbox();

			currentTimingShown.cameras = [camHUD];
			comboSpr.cameras = [camHUD];
			rating.cameras = [camHUD];
			confetti1.cameras = [camHUD];
			confetti2.cameras = [camHUD];
	...
	override function beatHit()
	{
		...
		if (currentSection != null)
		{
			...
			else if ((dad.curCharacter == 'gf-heart' || dad.curCharacter == 'gf-ratgod') && !dad.animation.curAnim.name.startsWith('sing'))
				dad.dance(forcedToIdle, currentSection.CPUAltAnim);
		}
		...
		if (!endingSong && currentSection != null)
		{
			...
			if (StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == "mad-rat-alive"
				&& (dad.curCharacter == 'gf-heart' || dad.curCharacter == 'gf-ratgod'))
			{
				if (curBeat == 31 || curBeat == 251)
				{
